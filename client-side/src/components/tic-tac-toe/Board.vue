...
export default defineComponent({
    ...
    methods: {
        ...
        handleMouse(payload: number) {
            this.handleGamePlay(payload, 'Mouse');
        },
        handleGamePlay(cellIndex: number, device: string) {
        ...
        takeAction(player: string, cellIndex: number) {
            this.cells[cellIndex - 1].player = player;
        },
        findWinner() {
            const playerX = []; // array of cells which are filled by playerX
            const playerO = []; // array of cells which are filled by playerO
